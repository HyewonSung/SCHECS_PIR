...
//4.26e-13;//pow(2.,-55);
  ...
  int32_t kpl =(k+1)*l;
  std::cout << "kpl done" << std::endl;

  int Bgbit=env->bgbit;
   std::cout << "Bgbit done" << std::endl;

  double log_2m=ceil(log(m)/log(2));
   std::cout << "log_2m done" << std::endl;

  Torus32 log2m = static_cast<int>(log_2m);
   std::cout << "log2m done" << std::endl;

  double log_2mm=ceil(log(mm)/log(2));
   std::cout << "log_2mm done" << std::endl;

  Torus32 log2mm = static_cast<int>(log_2mm);
   std::cout << "log2mm done" << std::endl;
...
/* //original
  ...
  if (data == nullptr || data->coefs == nullptr) {
    std::cerr << "Memory allocation failed for data or data->coefs!" << std::endl;
    return -1;
}

  std::cout << "db size: " << db << " elements" << std::endl;
std::cout << "Approx. memory required: " << (db * sizeof(int64_t)) / (1024.0 * 1024.0) << " MB" << std::endl;
*/
...
//bigger n
//Torus64Polynomial* data = new Torus64Polynomial(n, "/mnt/usb/bigdata_mmap.bin");
Torus64Polynomial* data = new Torus64Polynomial(n, "/mnt/usb/bigdata_mmap.bin");
    ...
    std::cout << "Memory allocation success with file-backed mmap!" << std::endl;
...
/*
   ...
   if (indexdata == nullptr) {
    std::cerr << "Memory allocation failed for indexdata!" << std::endl;
    delete data;
    return -1;
}
    ...
    std::cout << "indexdata done" << std::endl;

*/

 // ✅ `mmap()`을 사용한 인덱스 데이터 저장
    Torus64Polynomial* indexdata = new Torus64Polynomial(N);

    if (!indexdata || indexdata->coefs == MAP_FAILED) {
        std::cerr << "Memory allocation failed for indexdata!" << std::endl;
        delete data;
        return -1;
    }
    ...
    std::cout << "indexdata initialization done" << std::endl;
